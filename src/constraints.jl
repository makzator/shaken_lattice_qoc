...
function IQAPhiConstraint(
    ...
    Q_comps = [NTidx.index(t, traj.components[IQ_name][2], zdim) for t=1:T]
    phi_comps = [NTidx.index(t, traj.components[phi_name][1], zdim) for t=1:T]
    A_comps = [NTidx.index(t, traj.components[A_name][1], zdim) for t=1:T]

    gI(I, A, phi) = I - A*cos(phi)
    gQ(Q, A, phi) = Q - A*sin(phi)
    ∂gI(A, phi) = [1., -cos(phi), A*sin(phi)]
    ∂gQ(A, phi) = [1., -sin(phi), -A*cos(phi)]
    gI_phiphi(A, phi) = A*cos(phi)
    gI_Aphi(phi) = sin(phi)
    gQ_phiphi(A, phi) = A*sin(phi)
    gQ_Aphi(phi) = -cos(phi)
    ...
    ∂g_structure = [(t, comps[t]) for t=1:T for comps in [I_comps, A_comps, phi_comps]]
    append!(∂g_structure, [(t+T, comps[t]) for t=1:T for comps in [Q_comps, A_comps, phi_comps]])
    @assert length(∂g_structure) == 6*T
    @views function ∂g(Z⃗)
        ∂ = Array{Float64}(undef, 6*T)
        Threads.@threads for t=1:T
            phi = Z⃗[phi_comps[t]]
            A = Z⃗[A_comps[t]]
            ∂[3*t-2:3*t] = ∂gI(A, phi)
            ∂[(3*t-2:3*t) .+ 3*T] = ∂gQ(A, phi)
        end
        return ∂
    end

    μ∂²g_structure = [(phi_comps[t], comps[t]) for t=1:T for comps in [A_comps, phi_comps]]
    @assert length(μ∂²g_structure) == 2*T
    @views function μ∂²g(Z⃗, μ)
        μ∂² = Array{Float64}(undef, 2*T)
        Threads.@threads for t=1:T
            phi = Z⃗[phi_comps[t]]
            A = Z⃗[A_comps[t]]
            μ∂²[2*t-1] = μ[t] * gI_Aphi(phi) + μ[t+T] * gQ_Aphi(phi)
            μ∂²[2*t] = μ[t] * gI_phiphi(A, phi) + μ[t+T] * gQ_phiphi(A, phi)
        end
        return μ∂²
    end
